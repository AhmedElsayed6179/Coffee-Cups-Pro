...
if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
    ...
    // حذف الصورة القديمة
    $stmt = $connection->prepare("SELECT profile_image FROM guests WHERE username = ?");
    ...
    // حفظ الصورة الجديدة
    $newName = uniqid("guest_", true) . "." . $ext;
    $target = $uploadDir . $newName;

    if (move_uploaded_file($file['tmp_name'], $target)) {
        $stmt = $connection->prepare("UPDATE guests SET profile_image = ? WHERE username = ?");
...
if (isset($_POST['delete'])) {
    ...
    $stmt = $connection->prepare("UPDATE guests SET profile_image = NULL WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $stmt->close();
